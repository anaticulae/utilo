...
def similar(expected: str, current: str, maxdiff=0.6) -> bool:
    ...
    if not isinstance(expected, utilo.ITERABLE):
        expected = [expected]
    expected = utilo.strip(*expected)
    expected = utilo.lower(*expected)
    expected = utilo.nowhitespace(*expected)
    if isinstance(current, utilo.ITERABLE):
        sim = any(
            similar(
                expected=expected,
                current=item,
...
def verysimilar(current: str, expected: str) -> bool:
    ...
    if isinstance(current, str):
        current = [current]
    current = utilo.nowhitespace(*current)
