...
def text_prepare(items: str) -> str:
    ...
    if utilo.iterable(items) and len(items) > 1:
...
def final_newline(text: str) -> str:
    ...
    'hello\n'
    """
    return text.rstrip() + utilo.NEWLINE
...
def inside(current: str, expected: str, maxdiff: float = 0.1) -> bool:
    ...
    for side in range(curlen):
        ...
        if utilo.similar(current, splitted, maxdiff=maxdiff):
            return True
    return False
