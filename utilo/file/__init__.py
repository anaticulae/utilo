...
def file_create_tmp(
    ...
    >>> assert file_create_tmp('temporary content', utilo.ROOT)
    """
    path = tmpfile(root)
    with utilo.file.securewrapper.open(
            path,
            mode='w',
            newline=utilo.NEWLINE,
            encoding=utilo.UTF8,
            private=private,
    ) as fp:
        fp.write(content)
...
@utilo.typechecker.rename(source='src', destination='dest')
def file_copy(
    src: str = None,
    dst: str = None,
    update: bool = True,
    ...
    """
    utilo.exists_assert(src)
    # TODO: THINK ABOUT THIS
    assert not (private and unlock), 'do not unlock private files'
    try:
        if os.path.exists(dst) and not utilo.isfilepath(dst):
            # use name of current file as new file name
            # TODO: IS THIS REALLY NECESSARY?
            dst = os.path.join(dst, utilo.path_current(src))
        if update and file_compare(src, dst):
            # file is up to date
            return
        parent = utilo.path_parent(dst)
        os.makedirs(parent, exist_ok=True)
        # shutil.copy
        utilo.file.securewrapper.copy(src, dst, private=private)
        if timestamp:
            filetime = utilo.file_age(src)
            utilo.file_age_update(dst, filetime)
        if unlock:
            utilo.file_unlock(dst, noerror=True)
    except OSError as error:
        utilo.error(f'could not overwrite: {dst}')
        if exception:
            raise error
        utilo.exitx()
...
def file_list(  # pylint:disable=R1260
    ...
    for item in glob.glob(f'{path}/**/*', recursive=recursive):
        ...
        filepath = utilo.forward_slash(item)
        try:
...
def make_tmpdir(root: str, remove: bool = False, file_count_max: int = 100):
    ...
    if remove:
        ...
        assert len(utilo.file_list(path)) < file_count_max, msg
        utilo.tree_remove(path)
...
def assert_file(files, filetype: str):
    ...
    for item in files:
        if not item.endswith(f'.{filetype}'):
            utilo.error(f'No {filetype} file: {item}')
            raises += 1
