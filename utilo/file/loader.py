...
def from_raw_or_path(
    ...
    try:
        isdir = utilo.NEWLINE not in content and os.path.isdir(content)
    except ValueError:
...
def yaml_load(
    ...
    try:
        ...
        utilo.error('add `yaml` package to requirements, eg. `PyYAML>=5.1`')
        return None
    try:
...
def file_find(path, fnames, ftype=None):
    ...
    for fname in fnames:
        if ftype:
            ...
            fname = f'{fname}.{ftype}'
        newpath = utilo.join(path, fname)
        if not utilo.exists(newpath):
            continue
        return newpath
    return None
...
class LazyFile:
    ...
    def lazy(self):
        if self.content is not None:
            return self.content
        self.content = utilo.file_read(self.path).strip()
        return self.content
