...
def sync_pages(
    ...
    *,
    ...
    for index, iterator in enumerate(iterators):
        ...
        assert utilo.isascending(pages), (f'iter: {index}; pages: {pages} '
...
def determine_pagenumber(item) -> int:
    """\
    >>> determine_pagenumber([]) == utilo.INF
    True
    >>> determine_pagenumber(None) == utilo.INF
    True
    """
    if item is None:
        return utilo.INF
...
def simplify_pages(numbers: tuple) -> str:
    ...
    if isinstance(numbers, int):
        numbers = [numbers]
    collected = []
    diffed = utilo.groupby_diff(numbers, maxdiff=1)
    for item in diffed:
        if len(item) == 1:
            collected.append(item[0])
        else:
            collected.append(f'{item[0]}:{item[-1]+1}')
    joined = utilo.from_tuple(collected, separator=',')
