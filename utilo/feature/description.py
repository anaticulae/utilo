...
def prepare_description(
    ...
    description: str,
    workplan: list,
    features: 'utilo.feature.collector.FeatureInterfaces',
    ...
    return description + utilo.NEWLINE + raw


def format_inputs(step) -> str:
    inputs = []
    for source in step.inputs:
        if isinstance(source, utilo.Value):
            ...
            inputs.append(msg)
        elif isinstance(source, utilo.Bool):
            inputs.append(f'{source.name}(Bool)=True')
        elif isinstance(source, utilo.ResultFile):
