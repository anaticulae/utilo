...
def datapackage(call: str, version: str) -> str:
    try:
        ...
        return None
    sources = inputs(call)
    hashed = utilo.directory_hash(sources, ftype=('yaml', 'png', 'pdf'))
    if hashed is None:
        hashed = utilo.freehash([])
    simple = simple_call(call).strip()
    if ' ' in simple:
        program, parameter = simple.split(maxsplit=1)
    else:
        program, parameter = simple, ''
    parameter = utilo.freehash(parameter)
...
def use_cache(program, version, argv=None) -> int:
    ...
    cached = utilo.datapackage(call, version=version)
    if not cached:
        # no cache
        if not utilo.testing():
            utilo.debug('no cache')
        return False
    ...
    outpath = output(call)
    if not outpath:
        outpath = utilo.cwdget()
    os.makedirs(outpath, exist_ok=True)
    todo = f'python -m zipfile -e {cached} {outpath}'
    extracted = utilo.run(todo)
    return extracted.returncode == utilo.SUCCESS
...
def write_cache(program, version, argv=None) -> bool:
    ...
    if not outpath:
        outpath = utilo.cwdget()
    parent_cache = utilo.forward_slash(os.path.split(cached)[0])
    os.makedirs(parent_cache, exist_ok=True)
    collected = utilo.file_list(
        outpath,
        include=['yaml', 'png'],
    )
