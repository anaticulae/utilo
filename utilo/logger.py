...
def outfile():
    logfile = f'UTILA_LOG_OUTFILE_{utilo.mainthread()}'
...
def level_setup(level: Level):
    ...
    >>> level_setup(2)
    >>> assert utilo.level_current() == Level.INFO
    >>> level_setup(LEVEL_DEFAULT)
    """
    # TODO: REWORK THIS CONCEPT
    os.environ[f'UTILA_LOG_LEVEL_{utilo.mainthread()}'] = str(int(level))


def level_current() -> Level:
    try:
        level = os.environ[f'UTILA_LOG_LEVEL_{utilo.mainthread()}']
    except KeyError:
...
def log(
    *msg: str,
    ...
    """
    msg = utilo.from_tuple(msg) if isinstance(msg, tuple) else msg
...
def write_log(msg: str, end: str):
    ...
    with SINGLE_LOGGING:
        ...
        utilo.file_append(outfile(), msg + end, create=True)
...
def print_runtime(before: int, msg: str = '') -> float:
    ...
    Returns:
        Runtime diff
    """
    time_diff = utilo.roundme(time.time() - before)
    if msg:
...
class profile(contextlib.ContextDecorator):  # pylint:disable=C0103
    ...
    def __exit__(self, *exc_info):
        """Finish profiling."""
        if utilo.level_current() == Level.LOGGING and not self.always:
            return
...
def log_raw(content: str):
    ...
    Example:
        assert len(abc) > 100, utilo.log_raw(abc)
    """
...
def print_banner(text, symbol='*', width=80, newlines: bool = True):
    ...
    ******************************
    ...
    utilo.log(newlines + symbol * width)
    utilo.log(f'{symbol}{symbol}{text}{symbol}{symbol}')
    utilo.log(symbol * width + newlines)
...
def log_return(func):
    ...
    def logger(results):
        ...
        try:
            ...
            utilo.debug(f'{func.__name__} selected index: {index}')
        except ValueError:
            utilo.error(f'{func.__name__} select {selected} is not possible')
        return selected
