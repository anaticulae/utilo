...
def rect_width(rectangle: Rectangle) -> float:
    ...
    35
    """
    return utilo.roundme(rectangle[2] - rectangle[0])
...
def rect_height(rectangle: Rectangle) -> float:
    ...
    30
    """
    return utilo.roundme(rectangle[3] - rectangle[1])
...
def rect_max(items: list) -> tuple:
    ...
    x0 = utilo.mins(item[0] for item in items)
    x1 = utilo.maxs(item[2] for item in items)
    y0 = utilo.mins(item[1] for item in items)
    y1 = utilo.maxs(item[3] for item in items)
    return x0, y0, x1, y1
...
def rectangles_intersecting(
    ...
    True
    """
    return any(utilo.rect_intersecting(item, test) for item in rectangles)
...
def rect_overlapping(
    ...
    *,
    ...
    intersecting_size = utilo.rect_size(intersecting)
    if not intersecting_size:
        return 0.0
    size_test = utilo.rect_size(test)
    rate = intersecting_size / size_test
    rate = utilo.roundme(rate)
    if returnsize:
