...
def round_line(x0, y0, x1, y1, digits: int = 1, max_noise=4.0) -> tuple:
    ...
    if math.fabs(y0 - y1) < max_noise:
        y1 = (y1 + y0) / 2
        y0 = y1
    x0, y0, x1, y1 = utilo.roundme(x0, y0, x1, y1, digits=digits)
    return (x0, y0, x1, y1)
...
def intersecting_lines(first, second, max_diff=0.0):  # pylint:disable=R1260,R0914,R0912
    ...
    # if utilo.iszero(length(*first)) or utilo.iszero(length(*second)):
    ...
    if utilo.iszero(d):
        ...
        if matched:
            return matched
        if utilo.isequal(first[1], second[1]) and utilo.isequal(first[3], second[3]): # yapf:disable
    ...
    if utilo.isoutside(px, first[0], first[2], maxdiff=maxdiff) or\
       utilo.isoutside(px, second[0], second[2], maxdiff=maxdiff) or\
       utilo.isoutside(py, first[1], first[3], maxdiff=maxdiff) or\
       utilo.isoutside(py, second[1], second[3], maxdiff=maxdiff):
        return None
    return px, py
...
def merge_lines(items, diff: float = 3.0):
    ...
    for item in items[1:]:
        ...
        x0, y0 = item[0], item[1]
        if not all((
                utilo.near(last_x1, x0, diff=diff),
                utilo.near(last_y1, y0, diff=diff),
                utilo.near(
