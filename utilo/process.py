...
def run(
    ...
    cd...runtime(ls):...'0'
    """
    cwd = cwd if cwd else utilo.cwdget()
    ...
    with subprocess.Popen(  # nosec
        ...
        doit = utilo.profile if verbose else utilo.nothing
...
def exitx(msg='', returncode=utilo.utils.FAILURE):
...
def handle_run(
    ...
    try:
        ...
        utilo.error(f'timeout: {cmd} in {cwd} !failed!')
        proc.kill()
        if not gracefully:
            raise error
        if ontimeout:
...
def run_parallel(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=worker) as executor:
        ...
        for future in concurrent.futures.as_completed(todo):
            ...
            try:
                data = future.result()
                assert data.returncode == utilo.SUCCESS, data
            except Exception as exc:  # pylint:disable=broad-except
                utilo.error(f'{current} generated an exception: {exc}')
                ret += 1
    if expect:
        assert ret == utilo.SUCCESS, str(ret)
    if expect is False:  # pylint:disable=C2001
        assert ret >= utilo.FAILURE, str(ret)
    return ret
    ...
    with executor(max_workers=worker) as pool:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                utilo.error(f'future number: {index}; {future} failed.')
                utilo.error(error)
...
def returnvalue(exception: Exception) -> int:
    ...
    try:
        ...
        utilo.error(f'process does not provide exit value: {exception}')
        raise error
