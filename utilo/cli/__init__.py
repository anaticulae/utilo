    ...
    if '--version' in argv or '-v' in argv:
        ...
        try:
            ...
            utilo.exitx(
                ...
                returncode=utilo.INVALID_COMMAND,
            )
        utilo.exitx(version, returncode=utilo.SUCCESS)
...
def sources(  # pylint:disable=too-complex,too-many-branches
    ...
    *,
    ...
    """
    cwd = os.path.abspath(utilo.cwdget())
    ...
    if inputpaths:
        # make path absolute
        inputpaths = [utilo.make_absolute(item) for item in inputpaths]
...
def pages_fromargs(args) -> tuple:
    ...
    (0, 5, 10, 11, 12, 13, 14)
    """
    pages = args.get(PAGES_FLAG, [utilo.ALL_PAGES])
    joined = utilo.from_tuple(pages, separator=',')
    result = utilo.parse_pages(joined)
    return result
